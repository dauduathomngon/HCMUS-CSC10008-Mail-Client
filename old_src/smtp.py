...
class SMTP(Protocol):
    ...
    # Public method
    # --------------------------------------
    def create_connection(self, mail_from) -> bool:
        return (self.connect() and 
                self.__send_helo_command() and
                self.__init_sender(mail_from))
    ...
    def send(self, mail: MIMEMultipart):
        ...
        if send_msg[-2:] != bytes(DEFAULT.CRLF, "utf-8"):
            send_msg = send_msg + bytes(DEFAULT.CRLF, "utf-8")
        
        # tiến hành gửi mail
        self.__send_mail(mail_to, msg_bytes.getvalue())